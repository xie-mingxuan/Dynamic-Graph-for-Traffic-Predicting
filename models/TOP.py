...
class TOPModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.get_on_station_map = {}  # 用于预测在某个车站上车的概率，因此以下车站作为查询 key
        self.get_off_station_map = {}  # 用于预测在某个车站下车的概率，因此以上车站作为查询 key
        self.user_action_record = {}  # 用户上次的行为记录

        for i in range(1, 393):
            self.get_on_station_map[i] = []
            self.get_off_station_map[i] = []
        ...
        for src_node_id, dst_node_id, label in zip(src_node_ids, dst_node_ids, get_on_off_labels):
            ...
            if label == 0:
                ...
                if self.user_action_record[src_node_id]["label"] == 0:
                    ...
                    self.get_on_station_map[last_get_off_station].append(dst_node_id)
                    self.user_action_record[src_node_id]["station"] = dst_node_id
                ...
                if self.user_action_record[src_node_id]["label"] == 1:
                    ...
                    self.get_off_station_map[last_get_on_station].append(dst_node_id)
                    self.user_action_record[src_node_id]["station"] = dst_node_id
        ...
        for src_node_id, dst_node_id, label in zip(src_node_ids, dst_node_ids, get_on_off_labels):
            ...
            if label == 0:
                ...
                if self.user_action_record[src_node_id]["label"] == 0:
                    ...
                    last_get_off_station = self.user_action_record[src_node_id]["station"]
                    if last_get_off_station not in self.get_on_station_map:
                        possibilities.append(0.5)
                    else:
                        frequency = self.get_on_station_map[last_get_off_station].count(dst_node_id)
                        if len(self.get_on_station_map[last_get_off_station]) == 0:
                            possibilities.append(0.5)
                        else:
                            possibilities.append(frequency / len(self.get_on_station_map[last_get_off_station]))
                ...
                if self.user_action_record[src_node_id]["label"] == 1:
                    ...
                    last_get_on_station = self.user_action_record[src_node_id]["station"]
                    if last_get_on_station not in self.get_off_station_map:
                        possibilities.append(0.5)
                    else:
                        frequency = self.get_off_station_map[last_get_on_station].count(dst_node_id)
                        if len(self.get_off_station_map[last_get_on_station]) == 0:
                            possibilities.append(0.5)
                        else:
                            possibilities.append(frequency / len(self.get_off_station_map[last_get_on_station]))
                    if not negative_sampling:
                        self.user_action_record[src_node_id]["station"] = dst_node_id
                        self.user_action_record[src_node_id]["label"] = 1
