...
class GRUModel(nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: HistoricalInteractionSampler,
        ...
        self.gru_block = nn.ModuleList([
            nn.GRU(input_size=self.num_channels * self.channel_embedding_dim, hidden_size=self.num_channels * self.channel_embedding_dim, num_layers=1, batch_first=True)
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, node_interact_times: np.ndarray):
        ...
        for gru in self.gru_block:
            src_features, _ = gru(src_features)
            dst_features, _ = gru(dst_features)
