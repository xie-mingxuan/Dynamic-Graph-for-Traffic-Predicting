...
class LSTMModel(nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: HistoricalInteractionSampler,
                 time_feat_dim: int, channel_embedding_dim: int, num_layers: int = 2, dropout: float = 0.1,
                 max_input_sequence_length: int = 32, device: str = 'cpu'):
        """
        Sequence model.
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, node_interact_times: np.ndarray):
        ...
        src_nodes_neighbor_ids_list, src_nodes_edge_ids_list, src_nodes_neighbor_times_list = \
            self.neighbor_sampler.sample_interactions(node_ids=src_node_ids, node_interact_times=node_interact_times)
        ...
        #     self.neighbor_sampler.sample_interactions(node_id=dst_node_ids, timestamp=node_interact_times)

        dst_nodes_neighbor_ids_list, dst_nodes_edge_ids_list, dst_nodes_neighbor_times_list = [[] for _ in range(len(dst_node_ids))], [[] for _ in range(len(dst_node_ids))], [[] for _ in range(len(dst_node_ids))]
        ...
        # Transformer/Mamba/LSTM block processing
        for lstm in self.lstm_block:
            src_features, _ = lstm(src_features)
            dst_features, _ = lstm(dst_features)
    ...
    def set_neighbor_sampler(self, neighbor_sampler: HistoricalInteractionSampler):
        """
